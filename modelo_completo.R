...
pacotes <- c("plotly","tidyverse","ggrepel","fastDummies","knitr","kableExtra",
             ...
             "magick","cowplot","beepr","Rcpp","readxl", "ie2misc", "MLmetrics")
...
dados_gasolina_blr = read.csv("dados/gasolina_semanal_anp_blr.csv")
dados_acucar_blr = read.csv("dados/acucar_diario_rp_blr.csv")
...
dados_gasolina_blr$data <- as.Date(dados_gasolina_blr$data_fim,format="%d/%m/%Y")
dados_gasolina_blr$data_inicio <- NULL
dados_gasolina_blr$data_fim <- NULL

dados_acucar_blr$data_ajustada <- as.Date(dados_acucar_blr$data,format="%d/%m/%Y")
dados_acucar_blr$data <- dados_acucar_blr$data_ajustada + 1
dados_acucar_blr$data_ajustada <- NULL
...
chart.Correlation((dados[2:8]), histogram = TRUE)
...
#significâncias
dados %>%
  corr_plot(etanol_blr, gasolina_blr, acucar_blr, acucar_usd, dolar_blr, brent_usd, rbob_usd,
            shape.point = 21,
            col.point = "black",
            fill.point = "#FDE725FF",
            size.point = 2,
...
MAPE(dados$yhat_modelo_bc, dados$etanol_blr)


################################################################################
# CARGA E TRATAMENTO DE DADOS DE TESTE
################################################################################

dados_teste_etanol_blr = read.csv("dados_teste/etanol_semanal_anp_blr.csv")
dados_teste_gasolina_blr = read.csv("dados_teste/gasolina_semanal_anp_blr.csv")
dados_teste_acucar_blr = read.csv("dados_teste/acucar_diario_rp_blr.csv")
dados_teste_acucar_usd = read.csv("dados_teste/acucar_diario_ny_usd.csv")
dados_teste_dolar_blr = read.csv("dados_teste/dolar_diario_blr.csv")
dados_teste_brent_usd = read.csv("dados_teste/brent_diario_usd.csv")
dados_teste_rbob_usd = read.csv("dados_teste/rbob_diario_usd.csv")

dados_teste_etanol_blr$data <- as.Date(dados_teste_etanol_blr$data_fim,format="%d/%m/%Y")
dados_teste_etanol_blr$data_inicio <- NULL
dados_teste_etanol_blr$data_fim <- NULL

dados_teste_gasolina_blr$data <- as.Date(dados_teste_gasolina_blr$data_fim,format="%d/%m/%Y")
dados_teste_gasolina_blr$data_inicio <- NULL
dados_teste_gasolina_blr$data_fim <- NULL

dados_teste_acucar_blr$data_ajustada <- as.Date(dados_teste_acucar_blr$data,format="%d/%m/%Y")
dados_teste_acucar_blr$data <- dados_teste_acucar_blr$data_ajustada + 1
dados_teste_acucar_blr$data_ajustada <- NULL

dados_teste_acucar_usd$data_ajustada <- as.Date(dados_teste_acucar_usd$data,format="%d/%m/%Y")
dados_teste_acucar_usd$data <- dados_teste_acucar_usd$data_ajustada + 1
dados_teste_acucar_usd$data_ajustada <- NULL
dados_teste_acucar_usd$Abertura <- NULL
dados_teste_acucar_usd$Maxima <- NULL
dados_teste_acucar_usd$Minima <- NULL
dados_teste_acucar_usd$Vol <- NULL
dados_teste_acucar_usd$Var <- NULL

dados_teste_dolar_blr$data_ajustada <- as.Date(dados_teste_dolar_blr$data,format="%d/%m/%Y")
dados_teste_dolar_blr$data <- dados_teste_dolar_blr$data_ajustada + 1
dados_teste_dolar_blr$data_ajustada <- NULL
dados_teste_dolar_blr$Abertura <- NULL
dados_teste_dolar_blr$Maxima <- NULL
dados_teste_dolar_blr$Minima <- NULL
dados_teste_dolar_blr$Vol <- NULL
dados_teste_dolar_blr$Var <- NULL

dados_teste_brent_usd$data_ajustada <- as.Date(dados_teste_brent_usd$data,format="%d/%m/%Y")
dados_teste_brent_usd$data <- dados_teste_brent_usd$data_ajustada + 1
dados_teste_brent_usd$data_ajustada <- NULL
dados_teste_brent_usd$Abertura <- NULL
dados_teste_brent_usd$Maxima <- NULL
dados_teste_brent_usd$Minima <- NULL
dados_teste_brent_usd$Vol <- NULL
dados_teste_brent_usd$Var <- NULL

dados_teste_rbob_usd$data_ajustada <- as.Date(dados_teste_rbob_usd$data,format="%d/%m/%Y")
dados_teste_rbob_usd$data <- dados_teste_rbob_usd$data_ajustada + 1
dados_teste_rbob_usd$data_ajustada <- NULL
dados_teste_rbob_usd$Abertura <- NULL
dados_teste_rbob_usd$Maxima <- NULL
dados_teste_rbob_usd$Minima <- NULL
dados_teste_rbob_usd$Vol <- NULL
dados_teste_rbob_usd$Var <- NULL

dados_teste = dados_teste_etanol_blr

dados_teste = merge(x = dados_teste, 
              y = dados_teste_gasolina_blr, 
              by = "data", all.x = TRUE)

dados_teste = merge(x = dados_teste, 
              y = dados_teste_acucar_blr, 
              by = "data", all.x = TRUE)

dados_teste = merge(x = dados_teste, 
              y = dados_teste_acucar_usd, 
              by = "data", all.x = TRUE)

dados_teste = merge(x = dados_teste, 
              y = dados_teste_dolar_blr, 
              by = "data", all.x = TRUE)

dados_teste = merge(x = dados_teste, 
              y = dados_teste_brent_usd, 
              by = "data", all.x = TRUE)

dados_teste = merge(x = dados_teste,
              y = dados_teste_rbob_usd,
              by = "data", all.x = TRUE)

dados_teste = dados_teste[dados_teste$data >= '2022-08-01', ]
dados_teste = dados_teste[dados_teste$data <= '2022-11-30', ]

dados_teste = na.omit(dados_teste)

result = predict(object = step_modelo_bc, 
                 newdata = dados_teste,
                 interval = "confidence", level = 0.95)

result_data = as.data.frame(result)

result_data$predicted_value <- (((result_data$fit * lambda_BC$lambda) + 1)) ^ (1 / lambda_BC$lambda)
result_data$fit <- NULL
result_data$lwr <- NULL
result_data$upr <- NULL

dados_teste = merge(x = dados_teste,
                    y = result_data,
                    by = 'row.names', all = TRUE)

dados_teste$error_value <- dados_teste$etanol_blr - dados_teste$predicted_value

MAPE(dados_teste$predicted_value, dados_teste$etanol_blr)

write.csv(dados_teste, "dados_teste/resultado_teste.csv", row.names=FALSE)

#Ajustes dos modelos: valores previstos (fitted values) X valores reais
dados_teste %>%
  ggplot() +
  geom_smooth(aes(x = etanol_blr, y = predicted_value, color = "Box-Cox"),
              method = "lm", se = F, formula = y ~ splines::bs(x, df = 5), size = 1.5) +
  geom_point(aes(x = etanol_blr, y = predicted_value),
             color = "#440154FF", alpha = 0.6, size = 2) +
  scale_color_manual("Modelo:", 
                     values = c("#440154FF")) +
  labs(x = "Previsão", y = "Atual") +
  theme(panel.background = element_rect("white"),
        panel.grid = element_line("grey95"),
        panel.border = element_rect(NA),
        legend.position = "bottom")
